...
function interactSelectedPoint() {
    for (var i = 0; i < counters.puntiNumerati.length; i++) {
        if (counters.puntiNumerati[i].getPoint().getIsMoving() &&
...
export function line() {
    ...
    if (operations.actions.insertLine) {
        if (counters.controls.mouse.clicks == 0) {
            counters.linee.push(new Line("", counters.mousePosition.x, counters.mousePosition.y));
            counters.controls.mouse.clicks++;
        }
        else {
            counters.linee[counters.linee.length - 1].addPoint(counters.mousePosition.x, counters.mousePosition.y);
        }
    }
}
function generateSolution(x, y) {
...
export function pencil() {
    ...
    if (operations.actions.draw) {
        if (counters.controls.mouse.click == 0) {
            counters.pencil.push(new Line("", counters.mousePosition.x, counters.mousePosition.y));
            counters.controls.mouse.click++;
        }
        else {
            counters.pencil[counters.pencil.length - 1].addPoint(counters.mousePosition.x, counters.mousePosition.y);
        }
    }
}
// =====================================
